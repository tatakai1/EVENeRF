...
def render_rays(
    ...
    rgb_samples, density_samples = model.entangled_net(rgb_feat, ray_diff, mask, pts, ray_d, pose_diff)
    # volume rendering to get final 2D output
    comp_func = model.entangled_net.module.cond_nerf.composite if hasattr(model.entangled_net, "module") else model.entangled_net.cond_nerf.composite
...
def get_cameras_diff(ray_batch, rgb_feat):
    ...
    pose_diff = compute_pose_diff(ray_batch['src_cameras'][:, : , -16:], ray_batch['camera'][:, None, -16:].repeat(1, pose_shape[1], 1))
